...
import random  # Added for synthetic data generation
...
def generate_synthetic_data(n=500):
    """Generates synthetic data so the model learns the rules even with empty DB."""
    synthetic_records = []
    print(f"⚙️ Generating {n} synthetic records for robust training...")
    
    for _ in range(n):
        # Randomize attributes
        att = random.uniform(40, 100)
        gpa = random.uniform(2.0, 10.0)
        fails = random.choices([0, 1, 2, 3], weights=[0.7, 0.2, 0.08, 0.02])[0]
        fees = random.choice([0, 1])
        sent = random.uniform(-1.0, 1.0)
        
        synthetic_records.append({
            'attendance_percentage': att,
            'gpa': gpa,
            'past_failures': fails,
            'fees_due_binary': fees,
...
def fetch_and_train_model():
    ...
    try:
        ...
        if data:
            ...
            for val in raw_items:
                ...
                # Clean inputs
                fees_str = str(val.get('fees_due', 'No')).strip().lower()
        ...
        print(f"⚠️ Error fetching real data (using synthetic only): {e}")

    # --- 2. Add Synthetic Data (CRITICAL FIX) ---
    # We ALWAYS add synthetic data to ensure the model learns the correct risk logic
    # regardless of how many real students exist.
    student_records.extend(generate_synthetic_data(500))
    
    df = pd.DataFrame(student_records)
    df = df[feature_columns]

    # --- 3. CALCULATE GROUND TRUTH RISK (The Logic) ---
    # Rules:
    # 1. Low GPA (<10) increases risk
    # 2. Fees Due adds massive risk (+30)
    # 3. Failures add risk (+10 per fail, max 2 considered)
    # 4. Low Attendance increases risk
    ...
    # Sentiment currently has 0 impact in this formula, which is fine.
    # If you want sentiment to affect risk, add it here.
...
def predict_risk():
    ...
    try:
        ...
        # Input Cleaning (Added .strip())
        fees_str = str(data.get('fees_due', '')).strip().lower()
        fees_binary = 1 if fees_str in ['yes', 'due', 'unpaid'] else 0
        ...
        # Fallback: If model predicts ridiculously low but fees are due, force it up.
        # This is a safety guardrail.
        if fees_binary == 1 and risk_score < 30:
            risk_score = 30.0
